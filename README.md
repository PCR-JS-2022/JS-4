...
0      1     2     3
```
Сперва нужно реализовать класс самой биржи – ``ExchangeObserver``. Биржа должна иметь 1 поле и 3 метода:
...
После этого вам предстоит создать класс ``Company``. У ``Company`` должны быть 4 поля и 1 метод:
